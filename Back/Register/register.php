...
if (isset($_POST["firstname"], $_POST["lastname"], $_POST["email"], $_POST["pwd"], $_POST["phonenumber"]) && 
    !empty(trim($_POST["firstname"])) && !empty(trim($_POST["lastname"])) && !empty(trim($_POST["email"])) && 
    !empty(trim($_POST["pwd"])) && !empty(trim($_POST["phonenumber"]))) {
...
$regex_email = "/^[a-zA-Z0-9-+._]+@[a-zA-Z0-9-]{2,}\.[a-zA-Z]{2,}$/";
if (!preg_match($regex_email, $_POST["email"])) {
    echo json_encode(["success" => false, "error"=> "Format email invalide"]);
    die;
}

$regex_pwd = '/^[A-Z](?=.*\d)(?=.*[@#$%^+=!])(?=.*[a-zA-Z])[a-zA-Z0-9@#$%^+=!]{7,}$/';
if (!preg_match($regex_pwd, $_POST["pwd"])) {
    echo json_encode(["success" => false, "error" => "Format mot de passe invalide"]);
    die;
}
...
if (isset($_FILES['avatar'])) {
    ...
    if ($file_error === 0) {
        ...
        if (in_array($file_ext, $allowed_ext)) {
            ...
            if (move_uploaded_file($file_tmp, $destination)) {
                $hash = password_hash($_POST["pwd"], PASSWORD_DEFAULT);
                $req = $db->prepare("INSERT INTO user (prenom, nom, email, mot_de_passe, telephone, avatar) 
                                     VALUES (:prenom, :nom, :email, :mot_de_passe, :telephone, :avatar)");
                $req->bindValue(":prenom", $_POST["firstname"]);
                $req->bindValue(":nom", $_POST["lastname"]);
                $req->bindValue(":email", $_POST["email"]);
                $req->bindValue(":mot_de_passe", $hash);
